...
class Request extends AbstractRequest implements RequestInterface
{
  ...
  /**
   * ฟังก์ชั่นสร้าง uniqid
   *
   * @return string
   */
  public function createUniqeID()
  {
    $uniqeID = uniqid();
    $_SESSION[$uniqeID] = time();
    return $uniqeID;
  }
   ...
   * ลบ uniqid
   */
  public function removeUniqeID()
  {
    $uniqeID = $this->globals(array('POST', 'GET'), 'uniqeID', null)->toString();
    if ($uniqeID !== null) {
      unset($_SESSION[$uniqeID]);
    }
  }

  /**
   * ฟังก์ชั่น ตรวจสอบ uniqe ID ที่มาจากฟอร์ม
